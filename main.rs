...
impl Transaction {
    ...
    fn new_profile_creation(user_id: String, timestamp: String, global_tx_id: String) -> Self {
        Transaction {
            transaction_type: TransactionType::ProfileCreation,
    ...
    fn new_profile_update(user_id: String, timestamp: String, global_tx_id: String) -> Self {
        Transaction {
...
impl Profile {
    ...
    fn update(&self, new_data: RawProfileData, key: &[u8; 32]) -> Profile {
        Profile::new(self.user_id.clone(), new_data, key)
    }
}
...
impl UserShard {
    ...
    fn create_profile(
        ...
        let profile = Profile::new(self.user_id.clone(), raw_data, key);
        let tx = Transaction::new_profile_creation(self.user_id.clone(), timestamp, global_tx_id);
        self.profile = profile.clone();
        self.transactions.push(tx.clone());
        ledger.add_block(vec![tx], Some(profile));
    }
    ...
    fn delete_profile(&mut self, ledger: &mut GlobalLedger, mock_profile_db: &mut Vec<Profile>, timestamp: String, global_tx_id: String) {
        let mut profile = ledger.find_profile(&self.user_id).unwrap_or_else(|| self.profile.clone());
        ...
        if let Some(db_profile) = mock_profile_db.iter_mut().find(|p| p.user_id == self.user_id) {
            db_profile.is_deleted = true;
        }
        ledger.add_block(vec![deletion_tx], Some(profile));
    }
    ...
    fn revoke_key(
        ...
        ledger.add_block(vec![revocation_tx], None);
    }
}
...
impl GlobalBlock {
    fn new(transactions: Vec<Transaction>, previous_hash: String, miner: &Miner, difficulty: usize, profile: Option<Profile>) -> Self {
...
impl GlobalLedger {
    ...
    fn add_block(&mut self, transactions: Vec<Transaction>, profile: Option<Profile>) -> String {
        ...
        let block = GlobalBlock::new(transactions, previous_hash, miner, self.difficulty as usize, profile);
    ...
    fn find_profile(&self, user_id: &str) -> Option<Profile> {
        self.chain.iter().rev().find_map(|block| {
...
fn main() {
    ...
    let start = Instant::now();
    let miner_name = ledger.add_block(vec![tx], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 1 mined by {} in {:.3}s", miner_name, duration);

    // Create profiles explicitly on-chain
    println!("Creating Alice's profile on-chain...");
    alice_shard.create_profile(
        &mut ledger,
        RawProfileData {
    ...
    let mut bob_shard = UserShard::new(
        ...
        Vec::new(),
        Vec::new(),
        mock_profile_db.iter().find(|p| p.user_id == "bob").unwrap().clone(),
    );
    println!("Creating Bob's profile on-chain...");
    bob_shard.create_profile(
        &mut ledger,
        RawProfileData {
            name: "Bob".to_string(),
            age: 30,
            bio: "Enjoys hiking and reading".to_string(),
            interests: vec!["hiking".to_string(), "reading".to_string()],
            location: "CA".to_string(),
        },
        &bob_symmetric_key,
        "2025-03-04".to_string(),
        "create_bob".to_string(),
    );
    ...
    let mut charlie_shard = UserShard::new(
        ...
        Vec::new(),
        Vec::new(),
        mock_profile_db.iter().find(|p| p.user_id == "charlie").unwrap().clone(),
    );
    println!("Creating Charlie's profile on-chain...");
    charlie_shard.create_profile(
        &mut ledger,
        RawProfileData {
            name: "Charlie".to_string(),
            age: 25,
            bio: "Loves music and travel".to_string(),
            interests: vec!["music".to_string(), "travel".to_string()],
            location: "NY".to_string(),
        },
        &bob_symmetric_key, // Using Bob's key for simplicity
        "2025-03-04".to_string(),
        "create_charlie".to_string(),
    );
    ...
    println!("Block 5 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![match_tx], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 6 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![message_tx1.clone()], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 7 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![message_tx2.clone()], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 8 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![photo_tx.clone()], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 9 mined by {} in {:.3}s", miner_name, duration);
    ...
    let duration = start.elapsed().as_secs_f64();
    let miner_name = ledger.get_chain().last().unwrap().miner_name.clone();
    println!("Block 10 mined by {} in {:.3}s", miner_name, duration);
    ...
    let duration = start.elapsed().as_secs_f64();
    let miner_name = ledger.get_chain().last().unwrap().miner_name.clone();
    println!("Block 11 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![block_tx], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 12 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![video_call_tx], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 13 mined by {} in {:.3}s", miner_name, duration);
    alice_shard.interactions.push(Interaction {
    ...
    let miner_name = ledger.add_block(vec![report_tx1], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 14 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![report_tx2], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 15 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![key_share_tx], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 16 mined by {} in {:.3}s", miner_name, duration);
    shared_symmetric_keys.insert(("bob".to_string(), "alice".to_string()), alice_symmetric_key);
    ...
    let miner_name = ledger.add_block(vec![message_tx3.clone()], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 17 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![message_tx4.clone()], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 18 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![voice_tx.clone()], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 19 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![gift_tx.clone()], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 20 mined by {} in {:.3}s", miner_name, duration);
    ...
    let miner_name = ledger.add_block(vec![date_tx.clone()], None);
    let duration = start.elapsed().as_secs_f64();
    println!("Block 21 mined by {} in {:.3}s", miner_name, duration);
    ...
    for (i, block) in ledger.get_chain().iter().enumerate() {
        ...
        if let Some(profile) = &block.profile {
            let key = shared_symmetric_keys.get(&(profile.user_id.clone(), profile.user_id.clone())).unwrap_or(&[0u8; 32]);
            if let Some(raw_data) = profile.decrypt(key) {
                println!("  Profile: {} - {:?}", profile.user_id, raw_data);
            } else {
                println!("  Profile: {} - Deleted", profile.user_id);
            }
        }
        for tx in &block.transactions {
